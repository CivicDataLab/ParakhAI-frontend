import type {
  BulkTestCase,
  BulkTestCaseResultRef,
  BulkTestCaseRisk,
  ModuleIssueCount,
} from '../types/bulk-evaluation';
...
export const mapAuditResultsToBulkTestCases = (
  ...
  const moduleIssueMap = new Map<string, { displayName: string; issueCount: number }>();
  ...
  const items = Array.from(grouped.entries()).map(([testId, results], index) => {
    ...
    return {
      ...
      moduleDisplayName: primary.task?.moduleDisplayName || primary.task?.module || 'UNKNOWN',
  ...
  const moduleIssueCounts = Array.from(moduleIssueMap.entries()).map(([moduleId, value]) => ({
    moduleId,
