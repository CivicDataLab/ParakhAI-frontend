...
import {
  ...
  isErroredEvaluationStatus,
  isPendingReviewEvaluationStatus,
  isPlaygroundEvaluationMode as isPlaygroundEvaluationModeFromConstants,
  parseAuditType,
} from '@/constants';
...
export const isAuditInProgress = (status: string | null | undefined): boolean => {
  ...
  return (
    normalized === EVALUATION_STATUS.IN_PROGRESS ||
    normalized === EVALUATION_STATUS.QUEUED ||
    normalized === EVALUATION_STATUS.PENDING
  );
};

export const isAuditPendingReview = isPendingReviewEvaluationStatus;

export const isAuditFailed = isErroredEvaluationStatus;

export const isPlaygroundEvaluationMode = isPlaygroundEvaluationModeFromConstants;

export const hasCompletedAuditResults = (audit: {
  status: string;
  completedAt: string | null;
}): boolean => audit.status === EVALUATION_STATUS.COMPLETED || Boolean(audit.completedAt);

export const canShowBulkSummaryAndResults = (audit: {
  status: string;
  completedAt: string | null;
}): boolean => hasCompletedAuditResults(audit) || isAuditPendingReview(audit.status);
...
export const canShowEvaluationResults = (
  ...
  (isPlayground ? hasCompletedAuditResults(audit) : canShowBulkSummaryAndResults(audit));

export const shouldStopPolling = (
  audit: { status: string; completedAt: string | null },
  isPlayground: boolean
): boolean =>
  hasCompletedAuditResults(audit) || (!isPlayground && isAuditPendingReview(audit.status));
...
const EVALUATOR_LABELS: Record<string, string> = {
  [AUDIT_TYPE.DOMAIN_AUDIT]: 'Domain Expert',
  [AUDIT_TYPE.CULTURAL_AUDIT]: 'Cultural Expert',
  [AUDIT_TYPE.TECHNICAL_AUDIT]: 'Technical Evaluator',
};

export const getEvaluatorLabel = (auditType: string | null | undefined): string => {
  const parsed = parseAuditType(auditType);
  if (parsed) return EVALUATOR_LABELS[parsed];
  return auditType || '--';
};

export const getModeLabel = getEvaluationModeLabel;
...
export const parseEvaluatorRecommendation = (
  ...
  if (config) {
    for (const key of ['recommendation', 'evaluatorRecommendation', 'evaluator_recommendation']) {
...
export const aggregateRiskFromMetricSummary = (
  metricSummary: Record<string, { risk_distribution?: RiskDistribution }> | null | undefined
): RiskDistribution => {
  let low = 0;
  let medium = 0;
  let high = 0;
...
export const aggregateRiskFromAuditResults = (auditResults: AuditResult[]): RiskDistribution => {
  let low = 0;
  let medium = 0;
  let high = 0;
  ...
  for (const result of auditResults) {
    ...
    const severity =
      mapRiskLevel(result.evaluatorRiskLevel) ?? mapRiskLevel(result.riskLevel) ?? 'LOW';
...
export const resolveRiskDistribution = (
  ...
  const topLevelTotal = fromTopLevel.LOW_RISK + fromTopLevel.MEDIUM_RISK + fromTopLevel.HIGH_RISK;
...
export const getRiskDistributionTotal = (distribution: RiskDistribution): number =>
  (distribution.LOW_RISK ?? 0) + (distribution.MEDIUM_RISK ?? 0) + (distribution.HIGH_RISK ?? 0);
