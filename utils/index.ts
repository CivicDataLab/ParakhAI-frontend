...
/** Assignment status label — shows PENDING for pending invitations. */
...
export function formatDateString(input: string | number | any, isHyphenated = false): string {
  const date = new Date(input);
  return isHyphenated
    ? new Date(
...
export const getSiteMapConfig = () => ({
  ...
  childCacheDuration: parseInt(process.env.FEATURE_SITEMAP_CHILD_CACHE_DURATION || '21600'),
});
...
export const isSitemapEnabled = () => {
  return process.env.FEATURE_SITEMAPS === 'true' || process.env.NODE_ENV === 'production';
};
