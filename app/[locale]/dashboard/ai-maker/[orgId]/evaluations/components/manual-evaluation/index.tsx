...
const ManualEvaluationFlow: React.FC<ManualEvaluationFlowProps> = ({
  ...
  useEffect(() => {
    ...
    if (draft.status === TEST_CASE_STATUS.FAILED && draft.issueRows.length > 0) {
  ...
  useEffect(() => {
    if (status === TEST_CASE_STATUS.FAILED && issueRows.length === 0) {
      setIssueRows([createEvaluationIssueRow()]);
    }
    if (status !== TEST_CASE_STATUS.FAILED) {
      setIssueRows([]);
    }
  ...
  const handleSubmitTestCase = async () => {
    ...
    try {
      if (status === TEST_CASE_STATUS.PASSED) {
        await request<{
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
          ...
          <div className="manual-eval-input-panel relative bg-white p-6 pb-16">
            ...
            {hasCalledModel && (
              <div className="absolute bottom-6 right-6 flex justify-end gap-3">
                <Button
                  kind="secondary"
                  onClick={() => setStatus(TEST_CASE_STATUS.PASSED)}
                  className={`rounded-[6px] ${status === TEST_CASE_STATUS.PASSED ? 'bg-[#26007b] text-white hover:bg-[#4003c4] hover:text-white' : ''}`}
                >
                  ✓ Passed
                </Button>
                <Button
                  kind="secondary"
                  onClick={() => setStatus(TEST_CASE_STATUS.FAILED)}
                  className={`rounded-[6px] ${status === TEST_CASE_STATUS.FAILED ? 'bg-[#26007b] text-white hover:bg-[#4003c4] hover:text-white' : ''}`}
                >
                  ✕ Failed
                </Button>
              </div>
        ...
        {hasCalledModel && status === TEST_CASE_STATUS.FAILED && (
        ...
        {hasCalledModel && status === TEST_CASE_STATUS.PASSED && (
