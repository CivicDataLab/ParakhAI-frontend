...
export function mapManualTestCases(rows: ManualTestCaseRaw[]): ManualTestCase[] {
...
export function normalizeIssueSeverity(severity?: string | null): IssueSeverity | null {
...
export function getIssueRiskTagColors(severity?: string | null): {
  fillColor: string;
  textColor: string;
} {
...
export function formatRiskLabel(severity: string | null | undefined, label: string): string {
...
export function getManualEvalWorkspaceStorageKey(orgId: string, auditId: string) {
...
export function readManualEvalWorkspaceDraft(
  ...
  try {
    const raw = window.sessionStorage.getItem(getManualEvalWorkspaceStorageKey(orgId, auditId));
    if (!raw) return null;
    ...
    return {
      selectedModule: typeof parsed.selectedModule === 'string' ? parsed.selectedModule : null,
      ...
      inputPrompt: typeof parsed.inputPrompt === 'string' ? parsed.inputPrompt : '',
      modelOutput: typeof parsed.modelOutput === 'string' ? parsed.modelOutput : '',
      latencyMs: typeof parsed.latencyMs === 'number' ? parsed.latencyMs : undefined,
      hasCalledModel: Boolean(parsed.hasCalledModel),
      status:
        parsed.status === TEST_CASE_STATUS.PASSED || parsed.status === TEST_CASE_STATUS.FAILED
          ? parsed.status
          : null,
...
export function clearManualEvalWorkspaceDraft(orgId: string, auditId: string) {
  ...
  try {
    window.sessionStorage.removeItem(getManualEvalWorkspaceStorageKey(orgId, auditId));
  } catch {
    // Ignore storage errors.
  }
}

const FALLBACK_MODULE_META: Record<string, { displayName: string; description: string }> = {
...
export function getTotalManualTestCaseCount(moduleProgress: ModuleProgress[] | undefined): number {
