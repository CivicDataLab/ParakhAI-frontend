...
const BulkEvaluationResults = ({
  ...
  // Derived live from items so counts update instantly when issues are edited
  const moduleIssueCounts = useMemo<ModuleIssueCount[]>(() => {
    ...
    return Array.from(map.entries()).map(([moduleId, v]) => ({
      moduleId,
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const result = await request<{ auditResults: AuditResult[] }>(
  ...
  return (
    <div className="bulk-evaluation-results mb-8">
      ...
      <div className="bulk-evaluation-pending-banner mt-4">
        <IconSparkles
          size={18}
          className="bulk-evaluation-pending-banner__icon shrink-0"
          aria-hidden
        />
        <Text variant="bodySm" className="text-gray-900">
          This evaluation has AI generated observations pending review by the
          evaluator.
        </Text>
      </div>
